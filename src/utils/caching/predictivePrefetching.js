...
class PredictivePrefetcher {
  ...
  async init() {
    try {
      ...
      console.log('🔮 Predictive prefetching system initialized');
    } catch (error) {
  ...
  async loadUserBehavior() {
    try {
      ...
      if (behaviorData) {
        this.userBehavior = new Map(behaviorData.entries || []);
        console.log(`📊 Loaded behavior data for ${this.userBehavior.size} patterns`);
      }
    } catch (error) {
  ...
  async performPredictivePrefetching(userId) {
    ...
    try {
      console.log('🔮 Starting predictive prefetching for user:', userId);
      ...
      console.log(`✅ Predictive prefetching completed for ${prioritizedCandidates.length} items`);
    ...
    const prefetchPromises = candidates.map(async candidate => {
      try {
        ...
        await queryClient.prefetchQuery({
          ...
          cacheTime: QUERY_CONFIGS[this.getConfigKey(candidate.type)]?.cacheTime || 10 * 60 * 1000
        });
        
        console.log(`✨ Prefetched: ${candidate.type}`);
        
      } catch (error) {
        console.error(`Failed to prefetch ${candidate.type}:`, error);
      } finally {
  ...
  async clearPrefetchingData() {
    ...
    try {
      await idbStore.delete('user_behavior');
      console.log('🧹 Prefetching data cleared');
    } catch (error) {
      console.error('Failed to clear prefetching data:', error);
    }
  }
}
