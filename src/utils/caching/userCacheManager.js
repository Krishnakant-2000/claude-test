...
export class UserCacheManager {
  ...
  async initializeUserCaches() {
    ...
    const initPromises = cacheNames.map(async (cacheType) => {
      try {
        ...
        console.log(`Initialized cache: ${cacheName}`);
      } catch (error) {
  ...
  async clearAllUserCaches() {
    ...
    console.log(`Cleared ${successful}/${cacheTypes.length} user caches`);
  ...
  async prefetchUserPosts() {
    try {
      await queryClient.prefetchQuery({
        ...
        queryFn: () => import('../../services/api/postsService').then(service => service.getUserPosts(this.userId)),
        staleTime: 5 * 60 * 1000,
      });
    } catch (error) {
  ...
  async prefetchUserProfile() {
    try {
      await queryClient.prefetchQuery({
        ...
        queryFn: () => import('../../services/api/userService').then(service => service.getUserProfile(this.userId)),
  ...
  async prefetchUserMessages() {
    try {
      ...
      console.log('Messages prefetch placeholder - service not yet available');
  ...
  async prefetchFollowedContent() {
    try {
      await queryClient.prefetchQuery({
        ...
        queryFn: () => import('../../services/api/postsService').then(service => service.getFollowingPosts(this.userId)),
        staleTime: 5 * 60 * 1000,
      });
    } catch (error) {
