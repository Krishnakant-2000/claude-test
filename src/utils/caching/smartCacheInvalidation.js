...
class SmartCacheInvalidator {
  ...
  async init() {
    try {
      ...
      console.log('🧹 Smart cache invalidation system initialized');
    } catch (error) {
      console.error('Failed to initialize smart cache invalidator:', error);
    }
  }
  ...
  async loadInvalidationRules() {
    try {
      const rules = await idbStore.get('cache_invalidation_rules');
      if (rules) {
        this.invalidationRules = new Map(rules.entries || []);
        console.log(`📋 Loaded ${this.invalidationRules.size} invalidation rules`);
      }
    } catch (error) {
  ...
  async performCacheHealthCheck() {
    ...
    try {
      ...
      console.log(`✅ Cache health check completed. Invalidated ${invalidationResults.totalInvalidated} queries`);
  ...
  async performSizeBasedCleanup(analysis) {
    ...
    if (totalMemoryMB > maxMemoryMB) {
      console.log(`🧹 Performing size-based cleanup. Current usage: ${totalMemoryMB.toFixed(2)}MB`);
      ...
      console.log(`📉 Freed ${(freedMemory / (1024 * 1024)).toFixed(2)}MB of cache memory`);
    }
  }
  ...
  async getUserBehaviorPatterns() {
    try {
      const behaviorData = await idbStore.get('user_behavior');
      if (!behaviorData) return new Map();
      
      const patterns = new Map();
      const behaviorEntries = new Map(behaviorData.entries || []);
      ...
      for (const [key, behavior] of behaviorEntries.entries()) {
        ...
        patterns.get(contentType).push(...behavior.patterns);
      }
  ...
  startPeriodicHealthChecks() {
    ...
    console.log(`⏰ Scheduled cache health checks every ${this.checkInterval / (1000 * 60)} minutes`);
  }
  ...
  async invalidateQueriesWithPredicate(predicate, reason = 'manual') {
    ...
    console.log(`🎯 Manually invalidated ${invalidated.length} queries`);
    return invalidated;
  }
  ...
  async clearInvalidationData() {
    ...
    try {
      await idbStore.delete('cache_invalidation_rules');
      console.log('🧹 Cache invalidation data cleared');
    } catch (error) {
  ...
  async forceCleanup(options = {}) {
    ...
    console.log(`✨ Force cleanup completed. Removed ${removedCount} queries`);
