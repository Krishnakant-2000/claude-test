...
class OfflineAnalyticsTracker {
  ...
  async init() {
    try {
      ...
      console.log('📊 Offline analytics tracker initialized');
  ...
  async initializeAnalyticsStorage() {
    ...
    console.log('📋 Analytics storage structures prepared');
  }
  ...
  async loadPendingEvents() {
    try {
      ...
      console.log(`📥 Loaded ${this.eventQueue.length} pending analytics events`);
    } catch (error) {
  ...
  trackEvent(eventType, eventData = {}, options = {}) {
    try {
      ...
      const analyticsEvent = {
        ...
        userId: eventData.userId || this.getCurrentUserId(),
        data: {
          ...eventData,
  ...
  async storeEventToIndexedDB(event) {
    try {
      await idbStore.set('analytics_events', event.id, event);
    } catch (error) {
  ...
  async syncAnalyticsEvents(forceCriticalOnly = false) {
    ...
    try {
      ...
      console.log(`✅ Analytics sync completed: ${totalSynced} successful, ${totalFailed} failed`);
  ...
  async updateLocalStorageAfterSync(batchResults) {
    try {
      ...
      console.log(`🗑️ Cleaned up ${syncedEventIds.size} synced analytics events`);
  ...
  startPeriodicSync() {
    ...
    console.log(`⏰ Scheduled analytics sync every ${this.syncInterval / (1000 * 60)} minutes`);
  }
  ...
  async forceSync() {
    ...
    console.log('🚀 Forcing immediate analytics sync...');
