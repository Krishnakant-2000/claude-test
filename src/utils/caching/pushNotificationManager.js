...
class PushNotificationManager {
  ...
  async init() {
    try {
      ...
      console.log(`   Permission: ${this.isPermissionGranted}`);
  ...
  async showSyncNotification(type, details = {}) {
    try {
      ...
      console.log(`🔔 Showed notification: ${type}`);
  ...
  canShowNotification(type) {
    ...
    if (rateLimit > 0) {
      ...
      if (timeSinceLastNotification < rateLimit) {
        console.log(`⏰ Rate limit active for ${type}. ${Math.round((rateLimit - timeSinceLastNotification) / 1000)}s remaining`);
        return false;
      }
    }
  ...
  handleNotificationClick(data) {
    ...
    console.log(`🔔 Notification clicked: ${notificationType}, action: ${action}`);
  ...
  async focusApp(path = '/') {
    if ('clients' in window.self) {
      ...
      const clients = await window.self.clients.matchAll({ type: 'window' });
      ...
      return window.self.clients.openWindow(path);
    } else {
  ...
  handleRetryAction(data) {
    ...
    if (window.offlinePostManager) {
      window.offlinePostManager.attemptImmediateSync();
    }
  ...
  async clearNotificationData() {
    ...
    if (this.registration) {
      try {
        ...
        console.log(`🧹 Cleared ${notifications.length} pending notifications`);
      } catch (error) {
  ...
  scheduleNotification(type, details, delay) {
    ...
    console.log(`⏰ Scheduled ${type} notification for ${delay}ms`);
  }
}
